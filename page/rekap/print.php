    ...
    <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                ...
                while($data = $tampilKas->fetch_assoc()) : ?>
								...
								<?php 
								$totalTT[] = $data['jumlah'];
								$total_keluarT[] = $data['keluar'];
								?>
                <?php endwhile; 
                $totmasuk = array_sum($totalTT);
                $totkeluar = array_sum($total_keluarT);
                ?>
                </tbody>
                <tfoot>
                	<tr>
                		<th colspan="4">Total Kas Masuk</th>
                		<td>Rp.<?= number_format($totmasuk); ?></td>
                	</tr>
                	<tr>
                		<th colspan="4">Total Kas Keluar</th>
                		<td>Rp.<?= number_format($totkeluar); ?></td>
                	</tr>
                	<tr>
                		<th colspan="4">Saldo Akhir</th>
                		<td>Rp.<?= number_format($totmasuk-$totkeluar); ?></td>
                	</tr>
                </tfoot>
            </table>
        </div>
